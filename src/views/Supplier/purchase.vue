  ...
  <div class="content">
    ...
    <el-dialog title="商品信息" :visible.sync="dialogFormVisible" class="">
      <div>
        ...
        >
          ...
          >
            <el-input-number
              v-model="ruleForm.purchase_quantity"
              @change="handleChange"
              :disabled="true"
              :min="1"
              :max="10000"
    ...
    <div class="same-tabl">
      ...
      >
        ...
        <!-- 表头 -->
        <el-table-column label="采购id" width="70" fixed>
          <template slot-scope="scope">
            <span style="margin-left: 10px">{{ scope.row.purid }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="进货单价" width="160">
          <template slot-scope="scope">
            <span style="margin-left: 10px">{{
              scope.row.purchase_price + `元/` + scope.row.unit
            }}</span>
          </template>
        </el-table-column>
        <el-table-column label="采购数量(个)" width="160">
          <template slot-scope="scope">
            <span style="margin-left: 10px">{{
              scope.row.purchase_quantity + scope.row.unit
            }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="采购日期" width="200">
          <template slot-scope="scope">
            <span style="margin-left: 10px">{{ scope.row.purchase_date }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" fixed="right" width="200">
          ...
          <template slot-scope="scope">
            <el-button
              ...
              ><i class="el-icon-edit">进货</i></el-button
            >
          </template>
        </el-table-column>
      </el-table>
...
export default {
  data() {
    ...
    return {
      ...
      rules: {
        ...
        product_category: [
          { required: true, message: '审核状态', trigger: 'blur,change' },
        ],
        creation_time: [
          {
            required: true,
  ...
  methods: {
      ...
      await this.axios({
        ...
        url: `/foo/Inventory/inventoryname/${row.product_name}`,
          ...
          this.axios({
            ...
            data: {
              Inventory:
                Number(data.data.Inventory) + Number(row.purchase_quantity),
              sales_quantity: Number(pro.sales_quantity),
              current_status:
                Number(data.data.Inventory) + Number(row.purchase_quantity) > 0
                  ? '正常'
                  : '亏空',
              inventory_total:
          ...
          this.axios({
            ...
            data: {
              ...pro,
              Inventory: Number(row.purchase_quantity)+Number(pro.Inventory),
              current_status:
                Number(row.purchase_quantity) > 0 ? '正常' : '亏空',
              inventory_total:
                Number(pro.sales_price) * Number(row.purchase_quantity),
              creation_times: `${this.moment(+new Date()).format(
                'YYYY年MM月DD日 HH:mm:ss'
              )}`,
            },
          ...
          this.axios({
          method: 'patch',
          url: `/foo/purchase/activitupdata`,
          data: {
            actstates: true,
            purid: row.purid,
          },
        }).then(() => {
          this.getcategory('审核通过')
          alert('已经送往仓库')
        })
      })
    },
    ...
    shenghe(row) {
      if (
        ...
        this.axios({
          method: 'patch',
          url: `/foo/purchase/activitupdata`,
          data: { audit_status: '审核中', purid: row.purid },
        }).then(() => {
          this.getactivity()
          alert('已送往审核')
        })
    ...
    Edit(id) {
      ...
      this.axios({
        method: 'get',
        url: `/foo/purchase/activityid/${id}`,
      }).then(({ data }) => {
      ...
      if (confirm('确认删除数据吗？')) {
        this.multipleSelection.forEach((value) => {
          ...
          this.alldata.forEach((v) => {
            ...
            if (value.purid == v.purid) {
              this.AllDelete(v.purid)
            }
          })
        })
        alert('已删除')
    ...
    getcategory(values) {
      this.axios({
        method: 'get',
        url: `/foo/purchase/categorys/${values}`,
      }).then(({ data }) => {
        if (data.state == 0) {
    ...
    AddActivity() {
      this.axios({
        method: 'get',
        url: `/foo/purchase/purchase/${this.ruleForm.product_name}`,
      }).then(({ data }) => {
        this.axios({
          method: `post`,
          url: `/foo/purchase/actives`,
    ...
    async setupActivity() {
      await this.axios({
        method: 'get',
        url: `/foo/purchase/actives/${this.admin.Adminid}`,
      }).then(({ data }) => {
  ...
  computed: {
    ...
    purchase_total() {
      return this.ruleForm.purchase_price * this.ruleForm.purchase_quantity
    },
  },
  watch: {},
  created() {
