  ...
  <div class="login_container">
    ...
    <div class="login_box">
      ...
      <div>
        ...
        <h1 style="text-align: center; margin: 4% 0">供应商</h1>
      </div>
      <!-- 登录表单区域 -->
      <!-- model 数据绑定 ref 数据引用-->
      <el-form
      ...
      >
        <!-- 用户名 -->
        <el-form-item prop="supusername">
          <el-input
            v-model="loginForm.supusername"
            prefix-icon="el-icon-user-solid"
          ></el-input>
        </el-form-item>
        <!-- 密码 -->
        <el-form-item prop="suppassword">
          <el-input
            show-password
            type="suppassword"
            v-model="loginForm.suppassword"
            prefix-icon="el-icon-lock"
          ></el-input>
        </el-form-item>
        <div id="canvas-input">
...
export default {
  ...
  methods: {
      ...
      for (let key in userInfoList) {
        if (Object.hasOwnProperty.call(userInfoList, key)) {
          ...
          if (
            ...
            this.$store.commit('Supplier/SET_COMPANY_AUTH_INFO', {
              ...userInfo,
              password: '',
            })
            flag = true
    ...
    async denlu() {
      await this.axios({
        method: 'post',
        url: '/foo/Supplier/logins',
        data: this.loginForm,
      }).then(({ data }) => {
        if (data.state == 0) {
          this.$store.commit('Supplier/SET_token', data.token)
          this.$store.commit('Supplier/SET_COMPANY_AUTH_INFO', data.results)
          if (this.$store.state.Supplier.Supplier.rolue == 'user') {
            this.$router.push('/SupplierHome/MyHome')
