  ...
  <div id="main">
    <div class="column-view">
      <div class="main-area article-area">
        ...
        <article>
          <div class="article-title">{{ Activity.actiname }}</div>
          <div class="author-info-block">
            ...
            <div class="author-info-box">
              ...
              <div class="meta-box">
                <time class="time"> {{ Activity.activitytime }} </time>
                <span class="views-count" style="">
                  ·&nbsp;&nbsp;阅读{{ Activity.readCount }}
                </span>
              </div>
            </div>
          </div>
          <div class="article-content">
            <div
              ...
              v-html="Activity.actiContentHtml"
            ></div>
          </div>
        </article>
        <br />
        ...
        <div class="sidebar-block related-entry-sidebar-block shadow">
          ...
          <div class="block-body">
            <div class="entry-list">
              <a
                class="item"
                v-for="item in Activityalldata.slice(0, 6)"
                :key="item.activityid"
              >
                <div class="entry-title" @click="news(item.activityid)">
                  {{ item.actiname }}
                </div>
              </a>
            </div>
  ...
  methods: {
    ...
    async initalldata() {
      await this.axios({
        ...
        this.Activityalldata = data.data.filter((item) => {
          if (item.actitype == [...this.Activity.actitype]) {
            return item
          } else return 1
        })
        for (let key in this.Activityalldata) {
          if (Object.hasOwnProperty.call(this.Activityalldata, key)) {
            const userInfo = this.Activityalldata[key]
            this.Activityids.push(userInfo.activityid)
          }
        }
      })
    },
    ...
    async prev() {
      let index = this.id
      let id = --index
      let maxid = this.Activityalldata.sort((a, b) => {
        return a - b
      })
      let aid = this.Activityids.some((item) => {
        return item == id
      })
      console.log(maxid)

      if (aid && id > 0) {
        if (id >= maxid[0].activityid) {
          this.news(id)
        }
      } else {
        if (id >= maxid[0].activityid) {
    ...
    async next() {
      ...
      let aid = [...new Set(this.Activityids)].some((item) => {
        return item == id
      })
      if (aid && id > 0) {
        if (id <= maxid[maxid.length - 1].activityid) {
          this.news(id)
        }
      } else {
    ...
    async readcount(id) {
      await this.axios({
        ...
        this.axios({
          method: 'patch',
          url: `/foo/uctivity/uctive`,
          data: { readCount: data.data.readCount + 1, activityid: id },
        }).then(() => {
          this.$nextTick(() => {
            this.init(this.id)
          })
    ...
    goBack() {
      this.$router.push('/UserHome/UserActivity')
    },
  },
  created() {
    this.init(this.id)
