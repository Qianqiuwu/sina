        ...
        <el-table-column align="center" fixed="right" width="300">
          ...
          <template slot-scope="scope">
            <el-button
              ...
              @click="chaigou()"
              :disabled="
                scope.row.audit_status === '未审核' ||
                scope.row.audit_status === '审核中' ||
                scope.row.audit_status === '审核未通过' ||
                scope.row.actstates == true
              "
              >采购</el-button
            >
            <el-tooltip :content="scope.row.audit_status" placement="top">
              <el-button size="mini" @click="shenghe(scope.row)"
                >审核</el-button
              >
            </el-tooltip>
            ...
            <el-button
              size="mini"
              type="danger"
              @click="handleDelete(scope.row.purid)"
...
export default {
  data() {
    ...
    return {
      ...
      //审核状态
      status: [
        { id: 1, audit_status: '未审核' },
        { id: 2, audit_status: '审核中' },
        { id: 3, audit_status: '审核通过' },
        { id: 4, audit_status: '审核未通过' },
      ],
      // 表单
      ruleForm: {
        /*  */
  ...
  methods: {
    ...
    getprname(values) {
      this.axios({
        method: 'get',
        url: `/foo/purchase/purchase/${values}`,
      }).then(({ data }) => {
