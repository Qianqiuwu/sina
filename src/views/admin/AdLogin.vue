  ...
  <div class="login_container">
      ...
      <div>
        ...
        >
          <el-form-item label="用户" prop="Adusername">
            <el-input v-model="ruleForm.Adusername"></el-input>
          </el-form-item>
          <el-form-item label="密码" prop="Adpassword">
            <el-input
              v-model="ruleForm.Adpassword"
          ...
          <el-form-item label="手机号" prop="Adcell">
            <el-input v-model="ruleForm.Adcell"></el-input>
          </el-form-item>
    ...
    <div class="login_box">
      ...
      <div>
        ...
        <h1 style="text-align: center; margin: 4% 0">管理员</h1>
      </div>
      <!-- 登录表单区域 -->
      <!-- model 数据绑定 ref 数据引用-->
      <el-form
      ...
      >
        <!-- 用户名 -->
        <el-form-item prop="Adusername">
          <el-input
            v-model="loginForm.Adusername"
...
export default {
  ...
  methods: {
    ...
    async setup() {
      await this.axios({
        method: 'post',
        url: '/foo/admin/regusera',
        data: { ...this.ruleForm, rolue: `admin` },
      }).then(({ data }) => {
        if (data.state == 0) {
          alert('注册成功！')
        } else alert('用户名重复')
    ...
    async denlu() {
      await this.axios({
        method: 'post',
        url: '/foo/admin/logins',
        data: this.loginForm,
      }).then(({ data }) => {
        if (data.state == 0) {
          this.$store.commit('admin/SET_token', data.token)
          this.$store.commit('admin/SET_COMPANY_AUTH_INFO', data.results)
          if (this.$store.state.admin.admin.rolue == 'admin') {
            // console.log(this.$store.state.admin.admin.rolue)
            this.$router.push('/AdHome/MyHome')
          }
        } else alert('登录失败')
      })
    },
